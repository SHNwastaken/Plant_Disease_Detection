...
def main():
    ...
    if st.session_state.get('stream', False):
        ...
        while st.session_state['stream']:
            # Use the fetch_image function for consistency and error handling
            image = fetch_image(esp32_cam_url) 
            
            if image is not None: # Check if image fetching was successful
                results = leaf_model(image)
                for result in results:
                    boxes = result.boxes.xyxy
                    confidences = result.boxes.conf
                    for box, confidence in zip(boxes, confidences):
                        # Use the confidence_threshold from the slider
                        if float(confidence) > confidence_threshold:
                            x1, y1, x2, y2 = map(int, box)
                            # Ensure coordinates are valid and box has area
                            if x1 < x2 and y1 < y2:
                                ...
                                if cropped_leaf.size > 0:
                                    ...
                                    # Split disease_class once
                                    parts = disease_class.split('___')
                                    plant_type = parts[0]
                                    condition = parts[-1] # Use -1 in case class name has multiple '___'
                                    ...
                                    # Use the pre-split condition variable
                                    cv2.putText(image, f"Disease: {condition}", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                stframe.image(image, channels="BGR")
            else:
                # Optional: Add a small delay if fetching failed before retrying
                time.sleep(0.5) 
                
            time.sleep(0.1) # Keep the small delay between frames
